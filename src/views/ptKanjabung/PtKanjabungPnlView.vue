...
import { fetchKanjabungProfitLoss } from '@/services/kanjabungService'
...
const loadReport = async () => {
  ...
  let result = await fetchKanjabungProfitLoss(buildParams(requestDate))
  ...
  if (result.source === 'live' && result.data.length === 0) {
    ...
    if (cachedDate && cachedDate !== requestDate) {
      const fallback = await fetchKanjabungProfitLoss(buildParams(cachedDate))
...
const exportReport = () => {
  ...
  exportMultiSheetExcel(
    ...
    `KANJABUNG_LabaRugi_${selectedDate.value}_Unit${selectedUnit.value}`,
  )
}
</script>

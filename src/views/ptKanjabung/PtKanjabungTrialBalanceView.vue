...
import { fetchKanjabungTrialBalance } from '@/services/kanjabungService'
...
const loadReport = async () => {
  ...
  let result = await fetchKanjabungTrialBalance(buildParams(requestDate))
  ...
  if (result.source === 'live' && result.data.length === 0) {
    ...
    if (cachedDate && cachedDate !== requestDate) {
      const fallback = await fetchKanjabungTrialBalance(buildParams(cachedDate))
...
const exportReport = () => {
  ...
  exportMultiSheetExcel(
    ...
    `KANJABUNG_NeracaPercobaan_${selectedDate.value}_Unit${selectedUnit.value}`,
  )
}
</script>

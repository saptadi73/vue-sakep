...
import { fetchBprsBalanceSheet } from '@/services/bprsService'
...
const loadReport = async () => {
  ...
  let result = await fetchBprsBalanceSheet(buildParams(requestDate))
  ...
  if (result.source === 'live' && result.data.length === 0) {
    ...
    if (cachedDate && cachedDate !== requestDate) {
      const fallback = await fetchBprsBalanceSheet(buildParams(cachedDate))
...
const exportReport = () => {
  ...
  exportMultiSheetExcel(
    ...
    `PTBPRS_Neraca_${selectedDate.value}_Unit${selectedUnit.value}`,
  )
}
</script>

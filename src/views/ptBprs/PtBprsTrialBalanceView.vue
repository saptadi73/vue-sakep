...
import { fetchBprsTrialBalance } from '@/services/bprsService'
...
const LAST_SUCCESS_DATE_KEY = 'bprs-tb:last-success-date'
...
const loadReport = async () => {
  ...
  let result = await fetchBprsTrialBalance(buildParams(requestDate))
  ...
  if (result.source === 'live' && result.data.length === 0) {
    ...
    if (cachedDate && cachedDate !== requestDate) {
      const fallback = await fetchBprsTrialBalance(buildParams(cachedDate))
...
const exportReport = () => {
  ...
  exportMultiSheetExcel(
    ...
    `PTBPRS_NeracaPercobaan_${selectedDate.value}_Unit${selectedUnit.value}`,
  )
}
</script>
...
<template>
  <section class="report-page">
    <header class="report-header">
      ...
      <p class="sub">Saldo debit dan kredit seluruh akun per tanggal pelaporan.</p>

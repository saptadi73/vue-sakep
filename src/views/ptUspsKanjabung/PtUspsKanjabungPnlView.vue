...
import { fetchUspsKanjabungProfitLoss } from '@/services/usppsKanjabungService'
...
const loadReport = async () => {
  ...
  let result = await fetchUspsKanjabungProfitLoss(buildParams(requestDate))
  ...
  if (result.source === 'live' && result.data.length === 0) {
    ...
    if (cachedDate && cachedDate !== requestDate) {
      const fallback = await fetchUspsKanjabungProfitLoss(buildParams(cachedDate))
...
const exportReport = () => {
  ...
  exportMultiSheetExcel(
    ...
    `USPPS_LabaRugi_${selectedDate.value}_Unit${selectedUnit.value}`,
  )
}
</script>

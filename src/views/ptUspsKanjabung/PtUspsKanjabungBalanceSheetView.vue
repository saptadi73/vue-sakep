...
import { fetchUspsKanjabungBalanceSheet } from '@/services/usppsKanjabungService'
...
const loadReport = async () => {
  ...
  let result = await fetchUspsKanjabungBalanceSheet(buildParams(requestDate))
  ...
  if (result.source === 'live' && result.data.length === 0) {
    ...
    if (cachedDate && cachedDate !== requestDate) {
      const fallback = await fetchUspsKanjabungBalanceSheet(buildParams(cachedDate))
...
const exportReport = () => {
  ...
  exportMultiSheetExcel(
    ...
    `USPPS_Neraca_${selectedDate.value}_Unit${selectedUnit.value}`,
  )
}
</script>
